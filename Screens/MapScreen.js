  ...
  fetchUser = () => {
    ...
    firebase
      ...
      .on("value", snapshot => {
        ...
        this.setState({userPhoneNo:userInfo.contact,range:userInfo.range})
      });
  };
  ...
  _getLocationAsync = async () => {
   ...
   calculateDistance() {
    if (this.state.region.latitude && this.state.region.longitude) {
      let lat_result = this.state.region.latitude + this.state.range;
      let long_result = this.state.region.longitude + this.state.range;
      let display =
        (lat_result > this.state.range || long_result) > this.state.range ? true : false;
    }
  }
